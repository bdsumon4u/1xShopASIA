@foreach($models as $model)
    <tr
        class="text-center p-3 whitespace-nowrap @if ($loop->index & 1) bg-gray-200 @endif hover:bg-gray-100 {{ $this->setTableRowClass($model) }}"
    ...
    >
        @foreach($columns as $column)
            @if ($column->isVisible())
                <td
                    class="text-center p-3 border-b border-gray-200 text-sm {{ $this->setTableDataClass($column->getAttribute(), data_get($model, $column->getAttribute())) }}"
